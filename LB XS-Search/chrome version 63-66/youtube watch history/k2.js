...
self.addEventListener('message', function(event) {
	...
	else if (event.data && event.data.requests && event.data.action === 'cacheAll') {
		// console.log('cacheAll');
		cacheAll(event.data.requests).then(function() {
...
function cacheAll(requests) {
	return new Promise(function(resolve) {
		caches.open(CACHE_NAME).then(function(cache) {
			var cacheEmAll = requests.map(function(requestMetadata) {
				return new Promise(function(urlResolve) {
					let request = new Request(requestMetadata.url, requestMetadata.properties)
					fetch(request).then(function(response) {
						cache.put(requestMetadata.cacheKey, response).then(urlResolve);
					});
				});
			});
